...
> Last updated: 2026-05-11 (mid-morning refresh) | Auto-refreshed via Cowork scheduled task
...
- `/blog/` — Blog index + ~53 posts total (10 new spokes added 2026-05-09 afternoon + 1 flagship SLV reroof guide added 2026-05-11; all have BlogPosting + FAQPage + HowTo JSON-LD schema):
  ...
  - `/blog/utah/` — 15 statewide SEO posts (roof-replacement-cost, emergency-roof-repair, how-long-does-roof-last, how-to-choose-a-roofer, best-roofing-materials, signs-you-need-new-roof, utah-hail-season, utah-roof-insurance-claims, utah-roof-maintenance-checklist, utah-roof-ventilation-guide, april-2026-wasatch-storm-roof-checklist, commercial-roofing-utah, gutter-installation-guide-utah, roof-financing-options-utah, spring-roof-inspection-utah, **roof-replacement-quote-checklist-utah-2026** [NEW 2026-05-09 — Reddit content-gap close, compounds aeo-76a17e0e])
...
- `/supabase/` — **NEW 2026-05-09 (PR #23), expanded 2026-05-10 (d9bd585).** Edge function source-of-truth, previously deployed-only. Now holds: (a) `functions/handle-lead/index.ts` — **v8** (Resend email backend, retains v7 OpenRouter / Gemini 2.0 Flash tier classifier — emergency / urgent / scheduled / general / spam routing); (b) `functions/lead-crm/index.ts` — **NEW 2026-05-10**, 127 lines, PIN-gated reads + writes for the `/leads` CRM page (auth mirrors `weekly-report`); (c) `functions/handle-lead/DEPLOY.md` (deploy + secret-rotation notes); (d) `migrations/20260510_add_attribution_columns.sql` — **NEW 2026-05-10**, adds gclid / fbclid / utm_{source,medium,campaign,term,content} / won_at / uploaded_to_{google_ads,meta_ads} cols to `leads` + `leads_status_check` CHECK constraint + partial indexes for offline-conversion sync. `.gitignore` excludes `supabase/.temp/` (CLI cache); `.vercelignore` should keep `supabase/` off the static CDN.
- `/leads.html` — **NEW 2026-05-10 (d9bd585).** Public deliverable (PIN-gated) at `/leads` (cleanUrls). 433-line dark-themed CRM dashboard: KPIs (new untouched / last 30d / open pipeline / won MTD / 10% commission MTD), filter+sort+search, click-to-edit panel for status / notes / job_value / commission, auto-commission at 10% of job_value, auto-stamps `won_at` on status=`won`. Auth: same PIN system as `/seo-report` (key=`frame-roofing-2026` + per-user PIN against `report_access` table — existing PINs work). `seo-report.html` user-bar has a "Leads CRM →" button linking to it. **Do NOT add `leads.html` to `.vercelignore` reflexively** — header comment in `.vercelignore` flags this.
...
- **Edge function:** `lead-crm` **NEW 2026-05-10** — PIN-gated reads + writes for the `/leads` CRM page; auth mirrors `weekly-report` (key=`frame-roofing-2026` + per-user PIN against `report_access` table — existing PINs work unchanged). Source at `supabase/functions/lead-crm/index.ts`.
- **DB table:** `leads` (id, created_at, name, email, phone, address, service, message, source_page, status, job_value, commission auto-calc 10%, plus 2026-05-10 attribution columns: gclid, fbclid, utm_{source,medium,campaign,term,content}, won_at, uploaded_to_google_ads, uploaded_to_meta_ads + `leads_status_check` CHECK constraint + partial indexes for offline-conversion sync — migration `20260510_add_attribution_columns.sql`).
- **CRM dashboard:** `/leads` (`leads.html`, **NEW 2026-05-10**) — PIN-gated dark-themed CRM. KPIs (new untouched / last 30d / open pipeline / won MTD / 10% commission MTD), filter+sort+search, click-to-edit panel for status / notes / job_value / commission. Auto-fills commission as 10% of job_value; auto-stamps `won_at` when status flips to `won`. `/seo-report` user-bar links to it.
...
- **Edge function URLs:** https://hdcflshhomzildwqlmwh.supabase.co/functions/v1/handle-lead · https://hdcflshhomzildwqlmwh.supabase.co/functions/v1/lead-crm
