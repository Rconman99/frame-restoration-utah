...
# Audit prefers the full feed (all 20+ reviews, written by update-google-reviews.py)
# and falls back to the carousel feed (top 8) if the full feed isn't present yet.
# The full feed exists only after the next update-google-reviews.py run completes.
REVIEWS_FULL_JSON = ROOT / "data" / "reviews-full.json"
...
ATTRIBUTES: list[tuple[str, tuple[str, ...]]] = [
    ...
    ("Free Estimate / Inspection", ("inspect", "free estimate", "free inspection",
...
def load_reviews() -> tuple[list[dict], str]:
    """Load review corpus. Prefer reviews-full.json (all reviews) for accuracy;
    fall back to reviews.json (carousel feed, top 8) if the full file is missing.
    Returns (reviews_with_text, source_label)."""
    if REVIEWS_FULL_JSON.exists():
        data = json.loads(REVIEWS_FULL_JSON.read_text(encoding="utf-8"))
        all_reviews = data.get("reviews", [])
        with_text = [r for r in all_reviews if (r.get("text") or "").strip()]
        if with_text:
            return with_text, "reviews-full.json (audit feed)"
    if not REVIEWS_JSON.exists():
        raise SystemExit(f"ERROR: neither {REVIEWS_FULL_JSON} nor {REVIEWS_JSON} found — run update-google-reviews.py first")
    data = json.loads(REVIEWS_JSON.read_text(encoding="utf-8"))
    reviews = [r for r in data.get("reviews", []) if (r.get("text") or "").strip()]
    if not reviews:
        raise SystemExit("ERROR: no reviews with text found — SerpAPI may have failed")
    return reviews, "reviews.json (carousel fallback)"
...
def main() -> int:
    ...
    print(f"Audit corpus: {len(reviews)} reviews with text from {source}")
    stats = score(reviews)
    gaps = gap_list(stats)
